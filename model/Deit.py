...
def deit_small_distilled_patch16_224(pretrained=True, img_size=(224,224), num_classes =1000, **kwargs):
    ...
    if pretrained:
        ...
        print(torch.cat([weight[:, :2, :], new_matrix], dim=1).shape)
...
@register_model
def deit_tiny_distilled_patch16_224(pretrained=True, img_size=(224,224), num_classes =1000, **kwargs):
    model = DistilledVisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=192, num_classes=num_classes, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
...
@register_model
def deit_base_distilled_patch16_224(pretrained=True, img_size=(224,224), num_classes =1000, **kwargs):
    model = DistilledVisionTransformer(
...
@register_model
def vit_small_patch16_224(pretrained=True, img_size=(224,224), num_classes =1000, **kwargs):
    ...
    if pretrained:
        ...
        new_matrix = resize(matrix).permute(0, 2, 3, 1).reshape([1, -1, weight.shape[-1]])
        print(new_matrix.shape)
        checkpoint['pos_embed'] = torch.cat([weight[:, :2, :], new_matrix], dim=1)
        print(torch.cat([weight[:, :2, :], new_matrix], dim=1).shape)
        # change the prediction head if not 1000
        # if num_classes != 1000:
        #     checkpoint["model"]['head.weight'] = checkpoint["model"]['head.weight'].repeat(5,1)[:num_classes, :]
